...
int main(int argc, char** argv) {
    ...
    //START OF TRAJECTORY PUBLISHING *********
    //xxxxxxxxxx   
    ...
    //Position arm for movement 1 towards goal location in cartesian coordinates of (0.2, 0.4, 0.0)
    
    //Indicates quadrant position of gear with respective to goal coordinates (where goal coordinates is treated as the origin)
    int indicator = 0;
    //case 3 (when gear is in quadrant 3, with goal coordinates at the origin)
    if(0.2 - g_perceived_object_pose.pose.position.x < 0 && 0.4 - g_perceived_object_pose.pose.position.y > 0){
    ...
    traj_publisher.publish(new_trajectory); 
    ros::Duration(arrival_time).sleep(); 
    ROS_INFO("done with second trajectory, getting ready for first movement towards goal location");
    ros::Duration(2.0).sleep();
 
    //xxxxxxxxxxxxxxxxxx

    //Movement 1 of gear towards goal location, arm pushes gear toward x = 0.2 while other coordinate values remain the same
    ...
    traj_publisher.publish(new_trajectory); 
    ros::Duration(arrival_time).sleep(); 
    ROS_INFO("done with third trajectory, will position robotic arm for last movement towards goal location");
    ros::Duration(2.0).sleep();
    //xxxxxxxxxxxxxxxxxx

    //Positioning arm for Movement 2 of gear towards goal location
    ...
    traj_publisher.publish(new_trajectory); 
    ros::Duration(arrival_time).sleep(); 
    ROS_INFO("done with fourth trajectory, still positioning arm for last movement...");
    //xxxxxxxxxxxxxxxx

    //Still positioning arm for movement 2 of gear towards goal location
    ...
    traj_publisher.publish(new_trajectory); 
    ros::Duration(arrival_time).sleep(); 
    ROS_INFO("done with fifth trajectory, still positioning arm for last movement...");
    //xxxxxxxxxxxxxxxx

    //Finish positioning arm for Movement 2 of gear towards goal location
    ...
    traj_publisher.publish(new_trajectory); 
    ros::Duration(arrival_time).sleep(); 
    ROS_INFO("done with sixth trajectory, arm is successfully positioned for last movement");
    ros::Duration(2.0).sleep();

    //xxxxxxxxxxxxxx

    //Movement 2 of gear towards goal location, goal location of (0.2,0.4,0.0) is reached
    ...
    traj_publisher.publish(new_trajectory); 
    ros::Duration(arrival_time).sleep(); 
    ROS_INFO("done with seventh (last) trajectory, gear has reached goal location!");
    //xxxxxxxxxxxxxxxx
